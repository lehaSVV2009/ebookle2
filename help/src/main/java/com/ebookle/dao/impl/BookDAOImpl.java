...
public class BookDAOImpl extends AbstractDAOImpl<Book, Integer> implements BookDAO {
    ...
    @Override
    public List<Book> searchAll(String searchString) {
        FullTextSession fullTextSession = Search.getFullTextSession(getSession());
        QueryBuilder qb = fullTextSession.getSearchFactory().buildQueryBuilder().forEntity( Book.class ).get();
        org.apache.lucene.search.Query query =
//                qb.keyword().onFields("description","title","chapters.title","chapters.text","tags.bookTag","category.name").matching(searchString).createQuery();
                qb.keyword().onFields("title").matching(searchString).createQuery();
